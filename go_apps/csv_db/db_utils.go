...
func pourActionLog(csvRawData [][]interface{}, txn pgx.Tx, ctx context.Context, tableName string) error {
	...
	_, err := txn.CopyFrom(ctx, pgx.Identifier{tableName}, []string{"created_at", "action", "user_name", "family_name", "given_name", "school_class_name", "school_name", "remote_address", "content_name"}, pgx.CopyFromRows(csvRawData))
